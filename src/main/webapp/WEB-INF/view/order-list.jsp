...
<body>
	<div class="container">
		<c:forEach var="tmpOrder" items="${orders}">
			...
			<c:if test="${not tmpOrder.status}">
				...
				<ul>
					<c:forEach var="orderItem" items="${tmpOrder.orderItems}">
						<li>Product Id : ${orderItem.product.id} | Product Name :
							${orderItem.product.name} | Quantity : ${orderItem.quantity}</li>
					</c:forEach>
				</ul>


				<c:url var="deliveryLink" value="/order/deliveryOrder">
